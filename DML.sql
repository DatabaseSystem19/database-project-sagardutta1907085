...
--Retrieve the order_id and order_amount from the Orders table.
...
--Retrieve the employee_name and salary from the Employee table sorted by salary in ascending order.
	SELECT employee_name, employee_salary FROM Employee ORDER BY employee_salary ASC;
--Retrieve the customer_name and customer_address from the Customer table sorted by customer_name in alphabetical order.
	SELECT customer_name, customer_address FROM Customer ORDER BY customer_name;
...
--Retrieve the product names and prices from the Inventory table where the price is between 10 and 30.
